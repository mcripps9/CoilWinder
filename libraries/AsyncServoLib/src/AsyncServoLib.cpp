...
void AsyncServo::Update()
{
	...
	if (_endTime < _currentTime)
	{
		_isMoving = false;
		write(_endPosition);
		if (Finish != nullptr) { Finish(); return; }
	}
...
void AsyncServo::Pause()
{
	_isMoving = false;
	unsigned long _currentTime = micros();
	_timeRemaining = ( _endTime - _currentTime ) / 1000;
	if ( _timeRemaining < 0 )
	{
		_timeRemaining = 0;
	}
...
void AsyncServo::Resume()
{
	Move( _endPosition, _timeRemaining, _oldFinish );
}
